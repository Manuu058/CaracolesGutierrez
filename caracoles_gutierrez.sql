...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 04-04-2026 a las 18:09:15
...
(1, 'Almacén Principal', 'Dirección pendiente', '000000000', 'Pendiente', 1, '2026-04-03 14:43:01', '2026-04-03 14:43:01'),
(2, 'Almacén Sevilla', 'Sevilla', '000000001', 'Pendiente', 1, '2026-04-03 14:43:01', '2026-04-03 14:43:01');
...
INSERT INTO `lotes` (`id`, `codigo_lote`, `producto_id`, `proveedor_id`, `precio_compra`, `fecha_compra`, `cantidad_inicial`, `stock`, `observaciones`, `estado`, `created_at`, `updated_at`) VALUES
(1, 'LOT-2026-0001', 1, 1, 5.50, '2026-01-10', 100.00, 90.00, 'Lote inicial de caracoles', 'ACTIVO', '2026-04-03 14:43:01', '2026-04-03 14:43:01'),
(2, 'LOT-2026-0002', 2, 2, 6.20, '2026-01-15', 80.00, 75.00, 'Lote inicial de cabrillas', 'ACTIVO', '2026-04-03 14:43:01', '2026-04-03 14:43:01');
...
INSERT INTO `movimientos_lote` (`id`, `lote_id`, `tipo_movimiento`, `cliente_id`, `fecha`, `cantidad`, `precio_venta`, `descripcion`, `usuario_id`, `referencia_externa`, `created_at`, `updated_at`) VALUES
(1, 1, 'SALIDA', 1, '2026-04-03 16:43:01', 10.00, 8.50, 'Salida inicial a cliente demo', NULL, NULL, '2026-04-03 14:43:01', '2026-04-03 14:43:01'),
(2, 2, 'SALIDA', 2, '2026-04-03 16:43:01', 5.00, 9.50, 'Salida inicial a cliente demo', NULL, NULL, '2026-04-03 14:43:01', '2026-04-03 14:43:01');

--
-- Disparadores `movimientos_lote`
--
DELIMITER $$
CREATE TRIGGER `trg_movimientos_lote_after_delete` AFTER DELETE ON `movimientos_lote` FOR EACH ROW BEGIN
    IF OLD.tipo_movimiento = 'ENTRADA' THEN
        UPDATE lotes
        SET stock = stock - OLD.cantidad,
            estado = CASE
                        WHEN (stock - OLD.cantidad) <= 0 THEN 'AGOTADO'
                        ELSE estado
                     END
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_movimientos_lote_after_insert` AFTER INSERT ON `movimientos_lote` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_movimientos_lote_before_delete` BEFORE DELETE ON `movimientos_lote` FOR EACH ROW BEGIN
    ...
    IF OLD.tipo_movimiento = 'ENTRADA' AND (v_stock_actual - OLD.cantidad) < 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se puede borrar la entrada porque dejaría stock negativo';
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_movimientos_lote_before_insert` BEFORE INSERT ON `movimientos_lote` FOR EACH ROW BEGIN
    DECLARE v_stock_actual DECIMAL(10,2);
    ...
    IF NEW.tipo_movimiento = 'SALIDA' AND NEW.cliente_id IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Las salidas deben estar asociadas a un cliente';
    END IF;
    ...
    IF NEW.tipo_movimiento = 'SALIDA' AND (v_stock_actual - NEW.cantidad) < 0 THEN
        SIGNAL SQLSTATE '45000'
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_movimientos_lote_before_update` BEFORE UPDATE ON `movimientos_lote` FOR EACH ROW BEGIN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Los movimientos de lote no se editan; crea uno nuevo o elimina el existente';
END
$$
DELIMITER ;
...
(3, 'Caldo', 'Caldo preparado', 3.00, 'litros', 10.00, 1, '2026-04-03 14:43:01', '2026-04-03 14:43:01');
...
(1, 'Administrador', 'Sistema', 'admin@caracolesgutierrez.com', 'admin', '123456', '600000000', 1, '2026-04-04 17:25:23', '2026-04-03 14:43:01', '2026-04-04 15:25:23'),
...
ALTER TABLE `movimientos_lote`
  ...
  ADD KEY `idx_movimientos_cliente` (`cliente_id`),
...
ALTER TABLE `movimientos_lote`
  ...
  ADD CONSTRAINT `fk_movimientos_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
